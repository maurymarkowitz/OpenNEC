...
The basic idea is sound, but "real" NEC-2 allows comment cards to be placed only at the start of a file. Using this mechanism would separate the extensions from the cards they applied to, which would make the system more difficult to implement and reduce the quality of self-documentation. NEC-4 allows CM cards to be placed anywhere, but 3rd party implementations written before the release of NEC-4 generally do not support this. NEC-4 also introduced the ! character to allow comments to be added to the end of any line, but other implementations had already selected other characters to indicate comments, and very few of them, if any, support the `!` marker. Among the popular alternatives are nec2c and 4nec2, which use the `'` and `#` symbols in the same way NEC-4 uses `!`.
