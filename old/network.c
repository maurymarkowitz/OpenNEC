	...
	if( netcx.nonet != 0)
	{
	  ...
	  for( j = 0; j < netcx.nonet; j++ )
	  {
		...
		else
		{
		  ...
		  if (j < 3) {
			fprintf(stderr, "OLD nec2c TL[%d]: x11i=%g, wlam=%g, y22r_calc=%g, sin(y22r_calc)=%g, y12i=%g\n",
					j, netcx.x11i[j], data.wlam, TP* netcx.x11i[j]/ data.wlam, 
					sin(TP* netcx.x11i[j]/ data.wlam), y12i);
		  }
