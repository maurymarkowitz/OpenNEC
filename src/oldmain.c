...
//  OpenNEC
//
//  Created by Maury Markowitz on 2022-06-28.
//  Copyright © 2022 Maury Markowitz. All rights reserved.
//

// OLD MAIN METHOD, MOVED HERE FOR REFERENCE PURPOSES

//
//int mainrx( int argc, char **argv )
//{
...
//  if( read_line(line_buf, input_fp) == EOF )
//    abort_on_error(-3);
//
...
//    read_command_card( ain, &itmp1, &itmp2, &itmp3, &itmp4, &tmp1, &tmp2, &tmp3, &tmp4, &tmp5, &tmp6 );
//
...
//    for( ain_num = 0; ain_num < NUM_CONTROL_CODES; ain_num++ )
//    if( strncmp( ain, control_codes[ain_num], 2) == 0 )
//      break;
//
...
//
//        /* matrix solving  (network calls solves) */
