 ...
 * preprocess_max_min
 *
 * OpenNEC extension: Converts max() and min() calls to numbered versions.
 * tinyexpr supports fixed-arity functions (max1-max7, min1-min7).
 * This preprocessor converts "max(a,b,c)" to "max3(a,b,c)" based on argument count.
 * Limitation: maximum 7 arguments per max()/min() call (can nest for more).
 *
 * Returns a newly-allocated string that must be freed by the caller.
 */
char *preprocess_max_min(const char *formula) {
  if (!formula) return strdup("");
  
  char *result = strdup(formula);
  bool changed = true;
  
  // Keep processing until no more replacements are made (handles nested calls)
  while (changed) {
    changed = false;
    char *p = result;
    
    while (*p) {
      // Check for "max(" or "min(" (case-insensitive)
      bool is_max = (strncasecmp(p, "max(", 4) == 0);
      bool is_min = (strncasecmp(p, "min(", 4) == 0);
      
      if (is_max || is_min) {
        const char *func_name = is_max ? "max" : "min";
        char *func_start = p;
        char *open_paren = p + 3;  // Position of '('
        
        // Count the number of arguments by parsing balanced parentheses
        int arg_count = 1;
        int paren_depth = 1;
        ...
        while (*scan && paren_depth > 0) {
          if (in_string) {
            if (*scan == string_char && (scan == open_paren + 1 || *(scan - 1) != '\\')) {
              in_string = false;
            }
            scan++;
          } else {
            if (*scan == '"' || *scan == '\'') {
              ...
              paren_depth--;
              if (paren_depth == 0) {
                break;  // Found the matching close paren
              }
              scan++;
            } else if (*scan == ',' && paren_depth == 1) {
        ...
        // Validate we found the closing paren and arg count is in range
        if (paren_depth == 0 && arg_count >= 1 && arg_count <= 7) {
          // Build the replacement string
          char new_func[16];
          snprintf(new_func, sizeof(new_func), "%s%d", func_name, arg_count);
          
          // Reconstruct: prefix + new_func + "(" + content from open_paren to scan (inclusive)
          size_t prefix_len = func_start - result;
          size_t content_len = scan - open_paren + 1;  // Length from '(' to ')' inclusive
          
          // Allocate new result string
          size_t new_size = prefix_len + strlen(new_func) + content_len + 1;
          char *new_result = malloc(new_size);
          
          // Copy prefix (before "max"/"min")
          if (prefix_len > 0) {
            memcpy(new_result, result, prefix_len);
          }
          
          // Copy new function name
          memcpy(new_result + prefix_len, new_func, strlen(new_func));
          
          // Copy content from "(" onwards
          memcpy(new_result + prefix_len + strlen(new_func), open_paren, content_len);
          
          // Copy the rest of the string after the closing paren
          char *rest = scan + 1;
          strcpy(new_result + prefix_len + strlen(new_func) + content_len, rest);
          
          free(result);
          result = new_result;
          changed = true;
          break;  // Start over to handle any remaining nested calls
        } else {
          // Invalid or out-of-range argument count, skip to next position
          p++;
        }
      } else {
        p++;
