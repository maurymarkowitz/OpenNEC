...
void test_duplicate_tags(const context_t *ctx, const deck_t *deck, errors_list_t *errors)
{
  ...
  if (deck == NULL || errors == NULL)
  {
    return;
  }

  // Duplicate geometry tags are only invalid within the same section.
  // Cards in different NX sections may legitimately reuse a tag number.
  if (deck->num_sections > 0)
  {
    for (int s = 0; s < deck->num_sections; s++)
    {
      const section_t *section = deck->sections[s];
      if (section == NULL)
      {
        continue;
      }

      int gstart = section->geometry_start;
      int gend = section->geometry_end;
      if (gstart < 0 || gend < 0 || gend <= gstart)
      {
        // Malformed section: fall back to the section's card range and limit to geometry cards.
        gstart = section->global_start;
        gend = section->global_end;
      }
      if (gstart < 0 || gend < 0 || gend <= gstart)
      {
        continue;
        ...
        if (is_geometry(&deck->cards[i]) && card_has_itag(&deck->cards[i]) && deck->cards[i].i[1] > 0)
        {
          ...
          for (int j = i + 1; j < gend; j++)
          {
            if (is_geometry(&deck->cards[j]) && card_has_itag(&deck->cards[j]) && deck->cards[j].i[1] > 0)
            {
              ...
              {
                snprintf(msg, sizeof(msg), "The tag number %d is found on card %d and card %d in section %d.", tag_i, i + 1, j + 1, s + 1);
                add_error(ctx, errors, msg, 1);
              }
            }
          }
  ...
  int gend = DECK_GEOMETRY_END(deck);
  if (gstart < 0 || gend < 0 || gend <= gstart)
  {
    gstart = 0;
    gend = deck->num_cards;
