...
As part of this conversion, many of the shared variables used to pass information around nec2c were moved into malloc'ed structures. These are exposed and can be used by client software, instead of being hidden within the code as it was in the past. This is particularly useful during the antenna layout and design stage; programs can use the nec code to turn their geometry cards into segments which can then be sent to a 3D library. The performance is such that this can be run in realtime. It also means that most of the code is thread safe, due to the elimination of global variables.
...
- OpenNEC allows measurement units to be defined on a per-field basis. This is especially useful when defining wire radii; in an OpenNEC file a wire can be defined as "3awg" instead of having to replace that with the measurement in meters, "0.0058268". Different measurements can be used on different fields on the same card, or different cards. A `GS` card, if present, will *not* override fields with explicit measurements.
